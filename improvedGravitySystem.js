    ...
    (() => {
      ...
      const GRAVITY_SYSTEM = {
            ...
            for (const [timestamp, gravity] of riderKeyframes) {
              ...
              if (keyframeFrame === frameIndex) {
                ...
                if (gravity.compute === "set_gravity") {
                  ...
                  if (gravity._isPopImpulse && !gravity.compute) {
                    const duration =
                      typeof gravity._duration === "number"
                        ? gravity._duration
                        : 0;
                    const resetFrame = frameIndex + duration + 1;
                    this.scheduleGravityReset(
        ...
        getGravityForContact(riderIndex, contactPoint, frameIndex) {
          ...
          // Return current state for this contact point
          const gravityState =
            this.contactPointStates[riderIndex][contactPoint];

          // If we have a computed type, return as is
          if (gravityState && gravityState.type === "computed") {
            // For pop_impulse, we need to make sure the impulse is in the right format
            if (
              gravityState.compute === "pop_impulse" &&
              gravityState.impulse
            ) {
              return {
                type: "computed",
                compute: "pop_impulse",
      ...
      function generateRiderArray(originalRider, count, modifiers = {}) {
        ...
        return Array.from({ length: count }, (_, i) => {
          ...
          if (modifiers.startPosition)
            rider.startPosition = modifiers.startPosition(
              rider.startPosition,
              i,
            );
          if (modifiers.startVelocity)
            rider.startVelocity = modifiers.startVelocity(
              rider.startVelocity,
              i,
            );
          if (modifiers.startAngle)
          ...
          if (gravity && gravity.type === "computed") {
            ...
            switch (gravity.compute) {
              ...
              case "pop_impulse": {
                ...
                const impulseX = gravity.impulse
                  ? gravity.impulse.x
                  : gravity.x;
                const impulseY = gravity.impulse
                  ? gravity.impulse.y
                  : gravity.y;
      ...
      function calculateTargetPosition({
        ...
        let relativeOffset = shape[contactPoint];
      ...
      function getTargetVelocity(targetContactPoint, riderData) {
        if (
          targetContactPoint === null ||
          !riderData.points[targetContactPoint]
        ) {
          return { x: 0, y: 0 };
        }
      ...
      if (window.Actions) {
        ...
        const allGravityKeyframes = allRiders.map((r) =>
          r.getGravityKeyframes(),
        );
      ...
      window.GravitySystem = {
        ...
        debugStates: () => {
          ...
          console.log(
            ...
            GRAVITY_SYSTEM.lastProcessedFrame,
          );
        },
